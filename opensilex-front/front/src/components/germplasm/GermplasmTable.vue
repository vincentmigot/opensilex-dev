  ...
  <div class="card">
    ...
    <b-input-group class="mt-3 mb-3" size="sm">
      <downloadCsv
        ...
        name="template.csv"
      >{{$t('GermplasmTable.downloadTemplate')}}</downloadCsv>
      ...
      >{{$t('GermplasmTable.addRow')}}</b-button>
      <b-button
        class="mb-2 mr-2"
...
export default class GermplasmTable extends Vue {
  ...
  updateColumns() {
    ...
    let labelCol = {
      title: this.$t("GermplasmTable.label"),
      field: "name",
      visible: true,
      editor: true,
      minWidth: 150,
    ...
    let speciesCol = {
      title: this.$t("GermplasmTable.fromSpecies"),
      field: "fromSpecies",
      visible: true,
      editor: true
    };
    let varietyCol = {
      title: this.$t("GermplasmTable.fromVariety"),
      field: "fromVariety",
      visible: true,
      editor: true
    };
    let accessionCol = {
      title: this.$t("GermplasmTable.fromAccession"),
      field: "fromAccession",
      visible: true,
      editor: true
    };
  ...
  insertOrCheckData() {
    ...
    Promise.all(promises).then(result => {
      if (this.onlyChecking) {
        this.summary =
          this.okNumber +
          " germplasm ready to be inserted, " +
          this.errorNumber +
          " errors, " +
          this.emptyLines +
          " empty lines";
        ...
        .then((http: HttpResponse<OpenSilexResponse<any>>) => {
          if (onlyChecking) {
            this.tabulator.updateData([
              {
                rowNumber: index,
                checkingStatus: "ready to be inserted",
                status: "OK"
              }
            ]);
          } else {
            this.tabulator.updateData([
              {
                rowNumber: index,
                insertionStatus: "germplasm created",
                status: "OK",
